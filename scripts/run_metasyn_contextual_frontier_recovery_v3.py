#!/usr/bin/env python3
"""Prepare and execute the one-shot schema-compatible recovery-v3 smoke."""
...
from literature_multiverse.metasyn_contextual_frontier_recovery_v3 import (
    ...
    default_metasyn_contextual_frontier_recovery_client_v3,
    execute_metasyn_contextual_frontier_recovery_v3,
    freeze_metasyn_contextual_frontier_recovery_plan_v3,
    load_metasyn_contextual_frontier_recovery_plan_v3,
...
def _summary(value: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "original_schema_sha256": value.request.original_schema_sha256,
        "wire_schema_sha256": value.request.wire_schema_sha256,
...
def main() -> int:
    ...
    if args.command == "plan":
        value = freeze_metasyn_contextual_frontier_recovery_plan_v3(
            repository_root=args.repository_root, config_path=args.config
        )
    elif args.command == "prepare":
        value = prepare_metasyn_contextual_frontier_recovery_v3(
            repository_root=args.repository_root,
            workspace=args.workspace,
            config_path=args.config,
        )
    elif args.command == "authorize":
        value = authorize_metasyn_contextual_frontier_recovery_v3(
            workspace=args.workspace,
            phase_budget_usd_micros=args.phase_budget_usd_micros,
        )
    elif args.command == "execute":
        plan = load_metasyn_contextual_frontier_recovery_plan_v3(workspace=args.workspace)
        value = execute_metasyn_contextual_frontier_recovery_v3(
            repository_root=args.repository_root,
            workspace=args.workspace,
            client=default_metasyn_contextual_frontier_recovery_client_v3(plan),
        )
    else:
        value = load_metasyn_contextual_frontier_recovery_plan_v3(workspace=args.workspace)
