#!/usr/bin/env python3
"""Replay source authorizations and run the v3 synthesis-yield join."""
...
def _object(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"source_authorized_v3_json_not_object:{path}")
    return value
...
def main(argv: list[str] | None = None) -> int:
    ...
    if set(request) != {
        ...
        raise ValueError("source_authorized_v3_request_keys_mismatch")
    if request["request_version"] != "source-authorized-synthesis-v3-request-v1":
        raise ValueError("source_authorized_v3_request_version_unsupported")
    report = run_source_authorized_synthesis_v3(
