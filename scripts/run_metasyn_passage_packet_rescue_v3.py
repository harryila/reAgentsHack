#!/usr/bin/env python3
"""Run the additive exact-once MetaSyn passage-packet rescue v3."""
...
def _common(parser: argparse.ArgumentParser, *, anchor: bool = True) -> None:
    ...
    parser.add_argument("--workspace", type=Path, default=DEFAULT_RESCUE_WORKSPACE)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    smoke.add_argument(
        "--live",
        action="store_true",
        help="explicitly authorize the already-frozen at-most-three provider calls",
    )
...
def _client(args: argparse.Namespace, *, root: Path, v2_workspace: Path) -> AnthropicBoundedClient:
    if not args.live:
        raise MetaSynPassagePacketRescueV3Error("metasyn_packet_rescue_v3_live_flag_required")
    load_live_environment(_rooted(args.env_file, root), live_enabled=True)
    if not os.environ.get("ANTHROPIC_API_KEY"):
        raise MetaSynPassagePacketRescueV3Error(
            "metasyn_packet_rescue_v3_anthropic_api_key_missing"
        )
...
def main() -> int:
    ...
    if args.command == "prepare":
        value: Any = prepare_metasyn_passage_packet_rescue_v3(**common)
    else:
        anchored = {**common, "expected_plan_sha256": args.expected_plan_sha256}
        if args.command == "authorize":
            value = authorize_metasyn_passage_packet_rescue_v3(**anchored)
        elif args.command == "smoke":
            value = run_metasyn_passage_packet_rescue_smoke_v3(
                **anchored,
                client=_client(args, root=root, v2_workspace=v2_workspace),
            )
        elif args.command == "finalize":
            value = finalize_metasyn_passage_packet_rescue_v3(**anchored)
        elif args.command == "validate":
            value = validate_finalized_metasyn_passage_packet_rescue_v3(**anchored)
        else:
            value = metasyn_passage_packet_rescue_status_v3(**anchored)
